...
/*   Updated: 2017/01/14 18:34:43 by syusof           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		char *strend)
{
	...
	ft_init13(&dij1);
	dij1.strbegi = strbegi;
	dij1.strend = strend;
	ft_dijkstra_p1(&dij1, nodeprime, gene1);
...
{
	dij1->nodetmp3 = ft_cursref_first(nodeprime, dij1->strbegi);
	dij1->nodetmp3 = dij1->nodetmp3->nextr;
	while (dij1->nodetmp3)
	{
		if (ft_strcmp(dij1->nodetmp3->name, dij1->strend) == 0)
		{
			ft_node_add_wholer(&(gene1->nodescreen), dij1->nodetmp3, dij1->strbegi);
			(dij1->cnt)++;
		}
		else
...
void		ft_dijkstra_p2(t_dij *dij1, t_gene *gene1)
{
	...
	else
	{
		...
		if (ft_strcmp(dij1->nodetmp3->name, dij1->strend) == 0)
		{
			ft_node_add_wholer(&(gene1->nodescreen), dij1->nodetmp5,
					dij1->strbegi);
			dij1->indend = 1;
			ft_del_last(&(dij1->nodetmp5));
			dij1->cnt++;
		}
		else if (ft_strcmp(dij1->nodetmp3->name, dij1->strbegi) == 0)
			ft_del_last(&(dij1->nodetmp5));
	}
}
